...
Add a column called 'PRISON_SENTENCE' to the `offenders` dataframe. The column should contain a '1' if the offender received a prison sentence, or a '0' if the offender received a court order.

**Hint:** you'll need to apply the `dplyr::if_else()` function to the 'SENTENCE' column.
...
# Solution
offenders <- offenders %>%
  dplyr::mutate(PRISON_SENTENCE = dplyr::if_else(SENTENCE == "Court_order", 0, 1))
...
Add a column called 'PREV_CONVICTIONS_BAND' to the `offenders` dataframe. The column should contain the following categories: "0-1", "1-5", "5-10", "10+", based on the number of convictions given in the 'PREV_CONVICTIONS' column.
