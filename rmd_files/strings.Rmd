...
Similar to the wildcard you saw previously, ther are other reserved patters that serve a similar purpose for example:
...
A reminder here that if you want to use teh above to pattern then you will need to escape them as we learned earlier. So the `\d` string would be used as `\\d` in a pattern.
...
However, some characters will ahve a certain meaning even inside brackets and so the backslash for escaping them is still necessary, these are `] \ ^ -`.

Alternation is used to pick between patterns, for example `abc|d..f` will pick a patern with either `c` or `d` in the dfinition. Note how the `|` is used, it will only pick between the immediate characters and not between the entire patterns on either side. If it gets to a point where it becomes confusing remember to use parenthesis to clear things up. 
...
It is sometimes necessary for a certain pattern to appear multpiple time within a string, in such cases these repetitions can be coded with regular expressions to automate the search process. 
...
Another key aspect of the above code is that the number or precedence here dictates that the characterjust before the operater will be affected. This means that parenthesis will need to be used as in `bana(na)+` to capture more than one character.
...
To also note here that the system will match as many of the characters that it can find. To switch this behaviour off and use what is called "lazy" mathcing (instead of "greedy" as specified earlier) the `?` operator can be used as follows:
